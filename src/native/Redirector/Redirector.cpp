...
// Each successful initialization owns exactly one Winsock reference.
static bool winsockStarted = false;
static bool handlerStarted = false;
static bool filterStarted = false;

static void releaseRedirector()
{
	if (filterStarted) { nf_deleteRules(); nf_free(); filterStarted = false; }
	if (handlerStarted) { eh_free(); handlerStarted = false; }
