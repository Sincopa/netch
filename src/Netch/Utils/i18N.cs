...
public static class i18N
{
    ...
    public static void Load(string value)
    {
        ...
        switch (LangCode)
        {
            ...
            case "zh-CN":
                ...
                break;
            case "ru-RU":
                using (var stream = typeof(i18N).Assembly.GetManifestResourceStream("Netch.Resources.ru_RU.json")!)
    ...
    {
        var translateFile = new List<string> { "System", "en-US", "ru-RU", "zh-CN" };

        if (!Directory.Exists("i18n"))
            return translateFile;

        translateFile.AddRange(Directory.GetFiles("i18n", "*").Select(Path.GetFileName).OfType<string>());
        return translateFile.Distinct(StringComparer.OrdinalIgnoreCase).ToList();
    }
            ...
            component =>
            {
                switch (component)
                {
                    ...
                    case ToolStripItem toolStripItem:
                        toolStripItem.Text = Translate(toolStripItem.Text ?? string.Empty);
                        break;
