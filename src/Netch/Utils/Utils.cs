...
public static class Utils
{
    ...
    public static async Task<int> TCPingAsync(IPAddress ip, int port, int timeout = 1000, CancellationToken ct = default)
    {
        ...
        try
        {
            await client.ConnectAsync(ip, port, deadline.Token);
            return Math.Max(1, (int)Math.Ceiling(stopwatch.Elapsed.TotalMilliseconds));
        }
    ...
    public static async Task<int> ICMPingAsync(IPAddress ip, int timeout = 1000)
    {
        ...
        if (reply.Status == IPStatus.Success)
            return Math.Max(1, Convert.ToInt32(reply.RoundtripTime));
    ...
    {
        var form = System.Windows.Forms.Application.OpenForms.Cast<Form>().FirstOrDefault(value => !value.IsDisposed);
        if (form is null)
            return;
