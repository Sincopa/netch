...
public static class Socks5ServerTestUtils
{
    ...
    {
        using var timeout = CancellationTokenSource.CreateLinkedTokenSource(ctx);
        timeout.CancelAfter(Global.Settings.RequestTimeout);
        var proxy = new UriBuilder("socks5", socks5.Hostname, socks5.Port).Uri;
        var credentials = socks5.Auth() ? new NetworkCredential(socks5.Username, socks5.Password) : null;
        return await Netch.Application.ServerLatencyService.MeasureThroughProxyAsync(proxy, Global.Settings.LatencyTestUrl, timeout.Token, credentials);
    }
}
